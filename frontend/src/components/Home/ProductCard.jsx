...
const ProductCard = ({ image, originalPrice, discountedPrice, discountPercentage, name, rating, ratingCount, id }) => {
    ...
    return (
        <div className="w-[300px] mt-6 rounded-lg ml-4 overflow-hidden shadow-lg bg-white border cursor-pointer transform transition-transform hover:scale-105 relative flex flex-col">
            ...
            <img className="w-full h-[250px] object-cover" src={image} alt="Product" onClick={handleCardClick} />
            ...
            <div className="p-4 flex flex-col justify-between flex-grow">
                ...
                <div className="flex items-center space-x-2 mt-2">
                    {ownPlants == true ? "" :   discountedPrice && (
                        <p className="text-xl font-bold text-green-700">
                            Rs. {discountedPrice}
                        </p>
                    )}
                   {ownPlants == true ? "" :   discountPercentage && (
                        <span className="text-red-500 text-sm font-semibold">{discountPercentage}% off</span>
                    )}
                </div>
                {originalPrice && (
                    <p className={`text-gray-500 text-sm ${ownPlants ? "" : order ? "" :  "line-through"}`}>Rs. {originalPrice}</p>
                )}

                {
                    ownPlants ?(
                        <div className="flex justify-between mb-5 mt-5">
                            ...
                            <MdDelete onClick={delteHandler} className="inline text-red-600 cursor-pointer h-[24px] w-[24px]" />
                        </div>) : ""
                }

                {ownPlants == true ? "" : order ? (<button onClick={cancelOrderHandler}
